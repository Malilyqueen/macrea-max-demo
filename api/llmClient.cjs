const fetchLib = (typeof globalThis !== 'undefined' && globalThis.fetch) ? globalThis.fetch : null
let fetch = fetchLib
let AbortController = (typeof globalThis !== 'undefined' && globalThis.AbortController) ? globalThis.AbortController : null
try {
...
async function sendToLLM({ prompt, page, lead_profile, messages }) {
  ...
  try {
    ...
    console.error('[api/llmClient.cjs] provider=%s error=%s time_ms=%d', provider, err && err.message, duration)
    throw err
  }
}
